#Name: Chase-Miller
#Prog-Purpose: makes a coma data list into a smaller list.
...
def read_data_file():
    datafile = open(student_file,"r") #r means the READ file
    lines = datafile.readlines() # red the entire file
    for i in lines:
        stu.append(i.split(","))
    datafile.close()
...
def create_report_file():
    ...
    for i in range(len(stu)):
        ...
        lettergrade = str(stu[i][6])
        repfile.write("\n" + sname + hw + "\t" + midex + "\t" + finex + "\t" + fingrade + "\t" + lettergrade)
    repfile.write(line)
    repfile.close()
